...
static const uint8 g_client0_answer12[] = 
{
...
static const uint8 g_client0_answer13[] = 
{
...
//Packet 977
static const uint8 g_client0_answer14[] = 
{
		...
		if(CPacketUtils::HasPacket(incomingStream))
		{
			...
			for(const auto& subPacket : subPackets)
			{
				if((clientId == 0) && (subPacket.size() == 0x38) && (subPacket[0x10] == 0x00))
				{
					...
					{
						std::vector<uint8> outgoingPacket(std::begin(g_client0_answer13), std::end(g_client0_answer13));
						int sent = send(clientSocket, reinterpret_cast<const char*>(outgoingPacket.data()), outgoingPacket.size(), 0);
						assert(sent == outgoingPacket.size());
					}

					{
						std::vector<uint8> outgoingPacket(std::begin(g_client0_answer14), std::end(g_client0_answer14));
						int sent = send(clientSocket, reinterpret_cast<const char*>(outgoingPacket.data()), outgoingPacket.size(), 0);
						assert(sent == outgoingPacket.size());
					}
				}
